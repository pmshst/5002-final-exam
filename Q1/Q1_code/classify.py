...
def main():
    ...
    def outlier(inlier_train, outlier_train):
    
        if (outlier_train > inlier_train):
            return 1
        return 0
    
    df_submit = pd.read_csv('submit.csv')
    result = pd.DataFrame()
    result['ID'] = df_submit['id']
    result['Result'] = df_submit.apply(lambda x: outlier(x.inlier_train, x.outlier_train), axis = 1)
    
    result.to_csv('Q1_output.csv', index=None)
